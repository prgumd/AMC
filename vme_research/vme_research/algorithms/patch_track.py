...
def line_search_cond(x):
    norm_diff_I_T = x[0]
    new_norm_diff_I_T = x[1]
    steps = x[-1]
    return (new_norm_diff_I_T > norm_diff_I_T) & (steps < 10) # 1 / 256 = 0.00390625 # TODO don't hardcode

def line_search_step(x, I_W_p, W_W_inv):
    (norm_diff_I_T, new_norm_diff_I_T, I_warped_back_considered, alpha, p0, delta_p_inv, I, T_considered, p, delta_p, C_params, W_params, steps) = x

    delta_p_inv_scaled = delta_p_inv * alpha

    p, delta_p = W_W_inv(p0, delta_p_inv_scaled, *C_params)
    I_warped_back_considered = I_W_p(I, p, W_params)

    diff_I_T = I_warped_back_considered - T_considered
    new_norm_diff_I_T = jnp.linalg.norm(diff_I_T)

    new_alpha = alpha / 2
    x_ret = (norm_diff_I_T, new_norm_diff_I_T, I_warped_back_considered, new_alpha, p0, delta_p_inv, I, T_considered, p, delta_p, C_params, W_params, steps+1)
    return x_ret

# TODO the argument structure here is quite bad... try to fix
def flow_loop_body(inp, I, W_params, GN_params, C_params, I_W_p, W_W_inv, line_search_step):
    p_delta_p = inp[0]
    I_warped_back_considered = inp[1]
    norm_diff_I_T = inp[2]

    p0 = p_delta_p[0:(p_delta_p.shape[0] - 1) // 2]
    delta_p = p_delta_p[(p_delta_p.shape[0] - 1) // 2:(p_delta_p.shape[0] - 1)]
    steps = p_delta_p[-1]

    delta_p_inv = gauss_newton_step(I_warped_back_considered, *GN_params)

    # Poor man's line search
    new_norm_diff_I_T_0 = norm_diff_I_T + 1.0
    alpha0 = jnp.array(1.0)
    T_considered = GN_params[0]
    line_search_steps = jnp.array(0, dtype=jnp.int32)
    x = (norm_diff_I_T, new_norm_diff_I_T_0, I_warped_back_considered, alpha0, p0, delta_p_inv, I, T_considered, p0, delta_p, C_params, W_params, line_search_steps)
    x_ret = jax.lax.while_loop(
        line_search_cond,
        line_search_step,
        x,
    )
    (norm_diff_I_T, new_norm_diff_I_T, I_warped_back_considered, alpha, p, delta_p_inv, I, T_considered, p, delta_p, C_params, W_params, line_search_steps) = x_ret

    # TODO better linesearch termination, this doesn't work for some reason
    max_linesearches = line_search_steps == 10 # TODO don't hardcode
    p, delta_p = jax.lax.cond(
        max_linesearches,
        lambda x: (p0, jnp.zeros_like(delta_p)), # TODO does this work for all representations?
        lambda x: (p, delta_p),
        None
    )
    ...
    return jnp.array((*p, *delta_p, steps + 1)), I_warped_back_considered, new_norm_diff_I_T

def flow_loop(p, cond_parameters, body_parameters, cond, body, I_W_p):
    I = body_parameters[0]
    W_params = body_parameters[1]
    GN_params = body_parameters[2]
    I_warped_back_considered = I_W_p(I, p, W_params)
    T_considered = GN_params[0]
    diff_I_T = I_warped_back_considered - T_considered
    norm_diff_I_T = jnp.linalg.norm(diff_I_T)
    p_delta_p = jnp.concatenate((p, jnp.zeros(p.shape[0]+1,)))

    out = jax.lax.while_loop(
                       lambda x: cond(x[0], *cond_parameters),
                       lambda x: body(x, *body_parameters),
                       (p_delta_p, I_warped_back_considered, norm_diff_I_T))

    p_delta_p_steps = out[0]
    new_p = p_delta_p_steps[0:p.shape[0]]
    steps = p_delta_p_steps[2*p.shape[0]]
    return new_p, steps

def flow_loop_no_jit(p, cond_parameters, body_parameters, cond, body, I_W_p):
    I = body_parameters[0]
    W_params = body_parameters[1]
    GN_params = body_parameters[2]
    I_warped_back_considered = I_W_p(I, p, W_params)
    ...
    out = body((p_delta_p, I_warped_back_considered, norm_diff_I_T), *body_parameters) # We want a do while loop
    p_delta_p = out[0]
    I_warped_back_considered = out[1]
    norm_diff_I_T = out[2]

    while cond(p_delta_p, *cond_parameters):
        p_delta_p, I_warped_back_considered, norm_diff_I_T = body((p_delta_p, I_warped_back_considered, norm_diff_I_T), *body_parameters)
...
def affine_flow_loop_cond(p_delta_p, delta_p_stop, start_t, max_steps):
    ...
    steps = p_delta_p[2*N_p].astype(np.int32)
    return (steps == 0) | (jnp.linalg.norm(delta_p) > delta_p_stop) & (steps < max_steps)
...
affine_line_search_step = lambda *args: line_search_step(*args, affine_I_W_p, affine_compose_p_with_p_inv)
affine_flow_loop_body = lambda *args: flow_loop_body(*args, affine_I_W_p, affine_compose_p_with_p_inv, affine_line_search_step)
affine_flow_loop      = lambda *args: flow_loop     (*args, affine_flow_loop_cond, affine_flow_loop_body, affine_I_W_p)
...
homography_line_search_step = lambda *args: line_search_step(*args, homography_I_W_p, homography_compose_p_with_p_inv)
homography_flow_loop_body = lambda *args: flow_loop_body(*args, homography_I_W_p, homography_compose_p_with_p_inv, homography_line_search_step)
homography_flow_loop      = lambda *args: flow_loop     (*args, homography_flow_loop_cond, homography_flow_loop_body, homography_I_W_p)
...
def hom_4p_flow_loop_cond(p_delta_p, delta_p_stop, start_t, max_steps):
    N_p = (p_delta_p.shape[0] - 1) // 2
    delta_p = p_delta_p[N_p:2*N_p]
    steps = p_delta_p[2*N_p].astype(jnp.int32)
    return (steps == 0) | (jnp.linalg.norm(delta_p) > delta_p_stop) & (steps < max_steps)
...
hom_4p_line_search_step = lambda *args: line_search_step(*args, hom_4p_I_W_p, hom_4p_compose_p_with_p_inv)
hom_4p_flow_loop_body = lambda *args: flow_loop_body(*args, hom_4p_I_W_p, hom_4p_compose_p_with_p_inv, hom_4p_line_search_step)
hom_4p_flow_loop      = lambda *args: flow_loop     (*args, hom_4p_flow_loop_cond, hom_4p_flow_loop_body, hom_4p_I_W_p)
# hom_4p_flow_loop      = lambda *args: flow_loop_no_jit(*args, hom_4p_flow_loop_cond, hom_4p_flow_loop_body, hom_4p_I_W_p)
...
rot_line_search_step = lambda *args: line_search_step(*args, rot_I_W_p, rot_compose_p_with_p_inv)
rot_flow_loop_body = lambda *args: flow_loop_body(*args, rot_I_W_p, rot_compose_p_with_p_inv, rot_line_search_step)
rot_flow_loop      = lambda *args: flow_loop     (*args, rot_flow_loop_cond, rot_flow_loop_body, rot_I_W_p)
# rot_flow_loop      = lambda *args: flow_loop_no_jit(*args, rot_flow_loop_cond, rot_flow_loop_body, rot_I_W_p)
