...
def draw_warped_patch_location(frame, rect, p, R_fc_to_c, K):
    points = np.array([
        (rect[0], rect[1], 1),
        (rect[2], rect[1], 1),
        (rect[2], rect[3], 1),
...
def draw_full_reverse_warp(frame, rect, p, R_fc_to_c, K):
    ...
    cv2.polylines(frame_warped_back, [points], isClosed=True, color=(0, 255, 0), thickness=2)

    return frame_warped_back

# Affine flow calculation functions
...
def invert_delta_p(last_p, diff_T_I_shaped, grad_T_dW, H_T_dW_inv):
...
def affineLKTracker(frame,
    ...
    # Check the loss is lower:
    diff_T_I_shaped  = (T_considered - I_warped_back_considered).flatten()
    # loss = np.sum(np.square(diff_T_I_shaped))
    loss = np.linalg.norm(diff_T_I_shaped)
    if loss_prev is not None and loss > loss_prev:
        return False, loss, None, p, I_warped_back_considered
    ...
    if not inverse:
        ...
        delta_p, p = invert_delta_p(p, diff_T_I_shaped, grad_T_dW, H_T_dW_inv)
        #bit_end = time.time()

    #end = time.time()
    #print('end {:.2f} bit {:.2f} mid {:.2f}'.format(1000000*(end-bit_end), 1000000*(bit_end-mid), 1000000*(mid-start)))

    return True, loss, delta_p, p, I_warped_back_considered
...
def affine_flow_loop(frame,
    ...
    while True:
        #start = time.time()
        success, loss, delta_p_new, p_new, I_warped_back_considered = affineLKTracker(
                         frame,
                         T_considered,
                         rect,
                         region_size,
                         ...
                         H_T_dW_inv,
                         last_good_loss)
        # print('status', success, loss) #delta_p, p_new)
        if not success: # Do a poor mans line search
            delta_p *= 0.5
            p = compose_warp(last_good_p, delta_p)
        else:
            last_good_loss = loss
            last_good_p = p
            p = p_new
    ...
    sec_it = (t_current-t_start) / steps
    global sec_it_np
    sec_it_np.append(sec_it)

    if len(sec_it_np) > 100: 
        sec_it_np = sec_it_np[-100:]
    else:
        pass
       # print('not trimmed')

    # print('Update time {} steps {:0.4f} seconds {:0.1f} Hz jax'.format(steps, t_current - t_start, steps / (t_current-t_start)))
    # print('Update time {} steps {:0.4f} seconds {:0.1f} Hz mean {:0.1f} std dev {:0.1f}'.format(steps, t_current - t_start, steps / (t_current-t_start), 1000*np.mean(sec_it_np), 1000*np.std(sec_it_np)))
